# RadioPulse

RadioPulse is a Europe-wide commercial radio platform demo built with `.NET 10`, `ASP.NET Core`, `.NET Aspire`, `PostgreSQL`, `Redis`, `SignalR`, `ML.NET`, and `.NET MAUI`.

## Progress
- [x] Phase 0 - Repo scaffolding and standards
- [x] Phase 1 - Aspire orchestration and core services
- [x] Phase 2 - Data layer and domain
- [x] Phase 3 - Real-time engagement
- [x] Phase 4 - Now playing, schedule, podcast UX
- [x] Phase 5 - AuthN/AuthZ and security hardening
- [x] Phase 6 - Azure AI integration with local stub
- [ ] Phase 7 - ML.NET recommendations
- [ ] Phase 8 - .NET MAUI app
- [ ] Phase 9 - Docker, K8s, CI/CD
- [ ] Phase 10 - Polish and demos

## Repository Layout
- `src/` - .NET solution and projects
- `docs/` - architecture and decisions
- `.github/` - issue templates and CI/CD workflows

## Status
Phase 0 complete. See `docs/decisions.md` for current assumptions and defaults.
